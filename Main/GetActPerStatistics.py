...
def GetStatistic(_keyWordSeparate, _keySeparateInside, _traceList, _posToget, _posOfTime, _columnList):
    data = [
        [0 for _ in range(len(_columnList))],
        [0 for _ in range(len(_columnList))]
    ]
...
eventType = (para[1].split(_keySeparateInside)[-1])
posToGet = 0  # event to get = "Activity"
posOfTime = 2
if eventType == "Performer":
    posToGet = 1

totalMember = int(para[2].split(_keySeparateInside)[-1])
members = []
[members.append(para[3 + i].split(_keySeparateInside)[-1]) for i in range(totalMember)]
